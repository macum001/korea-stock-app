// jp: 앱 설정 페이지
// jp: Row/Toggle/Section 모두 파일 최상단 독립 컴포넌트 — SettingsPage 리렌더 시 재생성 방지
// jp: NicknameForm/PasswordForm도 독립 컴포넌트 + 명시적 key로 unmount 방지
...
// =====================================================================
// jp: 공통 UI — 파일 최상단 독립 선언 (SettingsPage 내부 X)
// =====================================================================
...
function ToggleUI({ on }: ToggleProps) {
  return (
    <span className="relative inline-block w-[42px] h-[24px] rounded-full transition-all"
      style={{ background: on ? 'linear-gradient(135deg,#7F77DD,#DB2877)' : 'var(--border)', flexShrink: 0 }}>
  ...
  return (
    <div style={{ padding: '12px 16px', background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border-subtle)' }}>
      <input
        ...
        placeholder={currentNickname || '새 닉네임 입력'}
        autoFocus
        onKeyDown={(e) => { if (e.key === 'Enter') void submit(); }}
      />
      {msg && <p style={{ fontSize: 11, marginTop: 6, color: msg.includes('됐어') ? '#4ADE80' : 'var(--fall)' }}>{msg}</p>}
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        ...
        <button onClick={submit} disabled={!ok || busy}
          style={{ flex: 1, padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 700, background: 'linear-gradient(135deg,#7F77DD,#DB2877)', color: '#fff', border: 'none', opacity: !ok || busy ? 0.5 : 1, cursor: !ok || busy ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
...
function PasswordForm({ onCancel, onSuccess }: PasswordFormProps) {
  ...
  const submit = async () => {
    if (!ok || busy) return;
    if (nxt.length < 6) { setMsg('새 비밀번호는 6자 이상이어야 해요.'); setIsErr(true); return; }
    ...
    try {
      ...
      setMsg('현재 비밀번호가 틀렸거나 오류가 발생했어요.');
      setIsErr(true);
    } finally { setBusy(false); }
  };
  ...
  return (
    <div style={{ padding: '12px 16px', background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      {msg && <p style={{ fontSize: 11, marginTop: 6, color: isErr ? 'var(--fall)' : '#4ADE80' }}>{msg}</p>}
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <button onClick={onCancel}
          style={{ flex: 1, padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 700, background: 'var(--bg-secondary)', color: 'var(--text-secondary)', border: 'none', cursor: 'pointer' }}>
          취소
...
export function SettingsPage() {
  ...
  return (
    <div style={{ minHeight: 'calc(100dvh - 60px - env(safe-area-inset-bottom))', background: 'var(--bg-primary)', paddingTop: 20 }}>
      ...
      <div className="mb-5">
        <p className="text-[11px] px-4 pb-1.5 pt-1" style={{ color: 'var(--text-tertiary)', letterSpacing: '0.4px' }}>알림</p>
      ...
      {isAuthenticated ? (
        <SettingSection title="계정">
          <div style={{ padding: '12px 16px', background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border-subtle)' }}>
            <p style={{ fontSize: 13, fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>{user?.nickname || '닉네임 없음'}</p>
          ...
          />
          {/* jp: key 고정 — showNickname 토글 시 unmount/remount 방지 */}
          {showNickname && (
            <NicknameForm
              key="nickname-form"
          ...
          <SettingRow icon={<LogOut size={18} />} label="로그아웃" onClick={logout} danger noBorder />
        </SettingSection>
      ) : (
        <SettingSection title="계정">
          <SettingRow
      ...
      <p style={{ textAlign: 'center', fontSize: 11, color: 'var(--text-tertiary)', paddingBottom: 24, marginTop: 8 }}>
        korea-stock-app · AI 기반 공시 분석 서비스
      </p>
